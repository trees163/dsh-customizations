	...
	factory: (require) => {
		...
		*/
		...
		var AgentPresetSeatController = class {
			...
			* @returns once the snapshot reflects the host.
			*
			* 本地补丁（2026-08-16）：新会话默认预设完全由设置（部署默认）决定，
			* 对话框显式选择（staged）优先；不再把"最近选中会话的预设"作为兜底
			* （原实现 currentSession()?.agentPreset 会拦截设置默认，导致不同
			* 工作区/会话流下"设置时灵时不灵"）。
			*/
			async load() {
				try {
			...
			*/
			async apply() {
				...
				// 修复（2026-08-16，本地补丁）：对话框选择（stage）应决定"下一个
				// 新建会话"的预设。原实现把"当前已有活动会话"与"已应用"混为
				// 同一分支并直接丢弃 staged —— 导致按钮显示所选预设（stage 已
				// 更新显示）但新会话仍落到部署默认（设置里的预设），形成
				// "选了不生效"的假象。现在：仅当会话已带着该预设（已应用）
				// 才消费 staged；遇到非空白会话时保留 staged，留给下一个空白
				// 会话（list-change applier 会在其成为 current 时再次 apply）。
